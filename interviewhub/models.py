...
class Interview(models.Model):
    ...
    confidentiality = models.CharField(
        max_length=16, choices=CONF_CHOICES, default=CONF_INTERNAL, help_text="How outputs are shared."
    )
    ...
    allowed_participants = models.ManyToManyField(
        settings.AUTH_USER_MODEL, related_name="invited_interviews", blank=True, help_text="Empty = any authenticated user."
    )
...
class Submission(models.Model):
    candidate = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="submissions", verbose_name="Participant"
    )
